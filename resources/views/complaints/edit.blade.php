    ...
    <div class="max-w-3xl mx-auto">
        ...
        <form action="{{ route('complaints.update', $complaint->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="bg-white rounded-lg shadow p-5">
                ...
                <div class="mt-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Foto Bukti Saat Ini</label>
                        ...
                        <div class="grid grid-cols-3 gap-3 mb-3">
                            @foreach ($complaint->photos as $photo)
                                <div class="relative">
                                    ...
                                    <form action="{{ route('complaints.photos.destroy', $photo->id) }}" method="POST"
                                        onsubmit="return confirm('Hapus foto ini?')" class="absolute top-1 right-1">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                                            class="bg-red-600 text-white w-6 h-6 rounded-full text-xs hover:bg-red-700">
                    ...
                    @else
                        <p class="text-sm text-gray-400 mb-3">Belum ada foto bukti.</p>
                    @endif

                    <label class="block text-sm font-medium text-gray-700 mb-1">Tambah Foto Baru (opsional)</label>
                    <input type="file" name="photos[]" accept="image/*" multiple
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                    <p class="text-xs text-gray-400 mt-1">Maks 2MB/foto, maksimal 5 foto per upload.</p>
                </div>
            </div>
